...
export default function AddIPHostModal({ onAdd, onClose }) {
  const [name, setName] = useState('')
  const [ipType, setIpType] = useState('IP')
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true)
    const err = await onAdd(result.payload)
    setLoading(false)
    if (err) setError(err)
    else onClose()
  }
      ...
      >
        ...
        <div className="px-5 pb-6 pt-2 sm:p-6">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-lg sm:text-xl font-bold text-gray-900">Add IP Host</h2>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <input
                ...
                placeholder="e.g. Office print servers"
                className="w-full border border-gray-300 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
              />
            </div>
